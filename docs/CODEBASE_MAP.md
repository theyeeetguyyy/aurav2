...
| `engine/scene/oklab.ts` | sRGB↔Oklab, hand-rolled. `L` is perceptual lightness and the colour lives in `(a, b)`, which is why a hue rotation there changes colour without changing brightness — the defect D-116 shipped with (D-120). |
| `engine/post/bricks/onebit.ts` | Dither, Halftone, ASCII. The Bayer matrix is computed rather than tabulated (a `const` array cannot be indexed by a varying in GLSL ES 1.0), halftone takes the square root of brightness for its dot radius, and the ASCII glyphs live in a `DataTexture` because a 5×5 mask needs 25 bits and a float is exact to 24 (D-121). |
| `effects/morph.ts` | Phase 4F, and the brief's headline requirement. A linear vertex lerp towards another procedural brick, which is only meaningful because the family shares one 642-vertex topology. An effect rather than an object property, so stack order decides morph-then-deform and Amount is routable for free (D-124). |
