...
export function CameraRigDriver() {
  ...
  useFrame(() => {
    const { behaviours, lookAtId, lookAtEnabled } = useCameraStore.getState()

    // A camera with no behaviours must not be dragged toward the origin by this code —
    // it stays exactly where the user authored it.
    if (!behaviours.some((b) => b.enabled)) {
      engine.holdScene()
      return
    }
    ...
    basePosition.copy(engine.baseScenePosition).sub(target)
