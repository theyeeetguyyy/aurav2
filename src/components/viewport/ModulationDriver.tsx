...
export function ModulationDriver() {
  useFrame(() => {
    ...
    // No filtering: a state owns its wires, so the loaded ones ARE the live ones. The timeline
    // swaps the whole set at a cut (see `TimelineDriver`) rather than masking a shared pool.
